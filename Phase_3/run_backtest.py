"""
run_backtest.py - Run Phase 2 backtest and save results.
...
print(f"STOP_LOSS_PCT: {STOP_LOSS_PCT*100:.0f}% (disabled)")
print("=" * 60)
...
def run_backtest(start_date, end_date, period_name):
    ...
    # Without rules
    print("\n🚀 Running backtest WITHOUT stop-loss and take-profit...")
    ...
    print(f"No Rules (No Stop-Loss/Take-Profit): £{final_no_rules:.2f}")
    print(f"Active Portfolio (With Rules):       £{final_active:.2f}")
...
def plot_performance_comparison(results, period_name, filename):
    ...
    ax.plot(results['no_rules'].index, results['no_rules'], 'r-', lw=2, label='No Rules (No Stop-Loss / Take-Profit)')
    ax.plot(results['active'].index, results['active'], 'b-', lw=2, label='Active Portfolio (With Rules)')
...
def plot_drawdown(results, period_name, filename):
    """Plot drawdown for no-rules portfolio."""
...
# Graph 4 & 5: Monthly returns (holdout)

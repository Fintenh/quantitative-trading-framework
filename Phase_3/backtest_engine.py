...
class BacktestEngine:
    ...
    def _empty_results(self):
        """Return empty results when backtest can't run."""
        print("  ⚠️ Backtest failed, returning empty results")
    ...
    def run(self):
        ...
        if len(dates) <= start_idx:
            print(f"  ⚠️ Not enough data: {len(dates)} days, need {start_idx + 1}")
            return self._empty_results()
        ...
        for i in range(start_idx, len(dates)):
            try:
                ...
                if i > start_idx:
                    # ============================================================
                    # FIX: Use original weights - DON'T re-normalise!
                    # Assets with NaN contribute 0% (no change)
                    # This prevents leverage on holidays
                    # ============================================================
                ...
                print(f"  ⚠️ Error on {dates[i]}: {type(e).__name__}: {e}")
                continue

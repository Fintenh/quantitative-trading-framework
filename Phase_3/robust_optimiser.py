"""
robust_optimiser.py - Bayesian optimisation, polynomial fitting, and robustness testing.
"""
...
class RobustOptimiser:
    ...
    def run(self, data, tickers, n_splits=5):
        ...
        if len(folds) < n_splits:
            print(f"\nOnly {len(folds)} folds available. Using all.")
            n_splits = len(folds)
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(folds[:n_splits]):
            ...
            # Skip folds where the training window is too short to be meaningful
            if len(fold_data['train']) < 100:
                print(f"\nSkipping fold {fold_idx + 1}: only {len(fold_data['train'])} training days")
                continue
            ...
            # If the Bayesian search produced no usable results, skip this fold
            # rather than fitting a polynomial to nothing.
            if bayesian_results is None or len(bayesian_results) == 0:
                print(f"  No Bayesian results for fold {fold_idx + 1}, skipping")
                continue
        ...
        # If every fold was skipped, fall back to the midpoint of each
        # parameter's search range rather than failing outright.
    ...
    def _run_bayesian(self, data):
        ...
        def objective(params):
            ...
            try:
                ...
                # Only print errors occasionally to avoid spamming the log
        ...
        # If every evaluation raised an exception, there is nothing to return
        if len(all_scores) == 0:
    ...
    def _fit_polynomial(self, results_df):
        """Fit a polynomial to the Bayesian results for each parameter."""
        param_names = list(self.param_space.keys())
        polynomial_results = {}

        # Fall back to the midpoint of each parameter's range if there is
        # nothing to fit a curve to.
        if results_df is None or len(results_df) == 0:
            print("  No results to fit a polynomial to, using defaults")
    ...
    def _multi_objective_score(self, metrics):
        """Calculate a multi-objective score for optimisation."""
    ...
    def get_results_dataframe(self):
        """Get fold results as a DataFrame."""

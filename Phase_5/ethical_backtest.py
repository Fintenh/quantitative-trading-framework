...
Kelly uses Maximum Sharpe portfolio with sign change detection for immediate rebalancing.
"""
...
from ethical_config import (
    ...
    LOG_DIR,
    FIGURES_DIR,
    # NEW: Import Kelly config values
    KELLY_BASE_CAP,
    KELLY_MAX_CAP,
)
...
# ============================================================================
# BACKTEST ENGINE WITH KELLY + SIGN CHANGE DETECTION
# ============================================================================

class BacktestEngineWithKelly(BacktestEngine):
    """
    BacktestEngine with Kelly + sign change detection.
    Forces immediate rebalance when f* changes sign.
    NOW USES CONFIG VALUES FOR KELLY CAPS (matching live engine).
    """
    ...
    def _calc_cash_allocation(self, returns):
        ...
        NOW USES CONFIG VALUES for Kelly caps (matching final_trading_engine.py).
        """
        try:
            ...
            # 2. Kelly cash cap - NOW USING CONFIG VALUES!
            # This matches final_trading_engine.py's behavior
            if f_star > 0:
                cash_cap = self.kelly_base_cap   # Uses config value (e.g., 0.20)
            else:
                cash_cap = self.kelly_max_cap    # Uses config value (e.g., 1.00)
            
            # 3. Sign change detection (triggers immediate rebalance)
            ...
            if self.prev_f_star is not None:
                ...
                if current_sign != prev_sign:
                    ...
                    self._force_rebalance = True  # ← IMMEDIATE REBALANCE!
        ...
        # 5. Cash allocation based on GARCH volatility (bounded by Kelly cap)
...
def run_backtest_for_universe(tickers, params, start_date, end_date, kelly_lookback, universe_name):
    ...
    engine = BacktestEngineWithKelly(
        ...
        kelly_base_cap=KELLY_BASE_CAP,    # ← PASS FROM CONFIG
        kelly_max_cap=KELLY_MAX_CAP,      # ← PASS FROM CONFIG
    )
    results = engine.run()

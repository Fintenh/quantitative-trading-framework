...
def log_portfolio(date, value, weights, cash_pounds, realised, total_wealth):
    ...
    for t in TICKERS:
        entry_full[f'{t}_value'] = value * weights.get(t, 0.0) if value > 0 else 0.0
    ...
    for t in TICKERS:
        entry_rounded[f'{t}_value'] = round(value * weights.get(t, 0.0), 2) if value else 0.0
...
def fetch_price_data_with_forward_fill(tickers, lookback_days):
    ...
    print(f"\nData summary: {len(df)} business days")
    return df
    ...
    for day in all_days:
        ...
        for t in TICKERS:
            if current_values[t] > 0:
                tp = today_prices.get(t, 0.0)
                yp = yesterday_prices.get(t, 0.0)
                if yp > 0 and tp > 0:
                    current_values[t] *= (tp / yp)
...
def should_rebalance(date, last_date, current, target, force_rebalance=False):
    """
    Determine whether a rebalance is needed. force_rebalance overrides the
    usual time/drift checks - used for take-profit events and Kelly sign
    changes, which should trigger a rebalance regardless of timing.
    """
    if force_rebalance:
        return True, "Force rebalance", 0.0, None
...
def compute_initial_state(tickers, unshifted_df, injection_date, price_date=None):
    """
    Compute the initial portfolio state. Prices are taken from the last
    business day before injection_date (rather than injection_date itself),
    since injection_date's own close isn't available yet at the point the
    engine first runs.
    """
    if price_date is None:
        # Get the last business day before injection_date
        available_dates = unshifted_df.index[unshifted_df.index < pd.Timestamp(injection_date)]
        if len(available_dates) > 0:
            price_date = available_dates[-1].date()
        else:
            # Fallback: use the first available date (shouldn't happen)
            price_date = unshifted_df.index[0].date()

    # Use the price date for returns
...
def main():
    ...
    # Flag to force rebalance (take-profit or Kelly sign change)
    force_rebalance = False

    if is_first:
        # First run: injection date = today, compute initial state using previous day close
        injection_date = today
        print(f"   First run - injection date: {injection_date}")
        ...
        print(f"   Starting at £{value:.2f} (Cash: £{cash_pounds:.2f}, {cash_percentage*100:.1f}%)")
        print(f"   Using price date: {price_date}")
        if value > 0:
            print("   Initial weights:")
            sorted_w = sorted([(t, weights[t]*100) for t in weights if weights[t] > 0.001], key=lambda x: x[1], reverse=True)
            for t, w in sorted_w[:10]:
                print(f"      {t}: {w:.1f}%")

        # Don't log the portfolio yet - let the rebalance logic below do it,
        # since it also needs to compute and log the target allocation.
        # Leave last_rebalance as None so the rebalance check triggers.
        last_rebalance = None

        # Save injection date for reference
        ...
        # Load last rebalance date (may be None if never rebalanced)
    ...
    # Combine force flags: take-profit or Kelly sign change
    force_rebalance = take_profit_triggered or kelly_sign_changed

    rebalance_needed, reason, drift, asset = should_rebalance(
        now_date, last_rebalance, weights, adjusted_target,

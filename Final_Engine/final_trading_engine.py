...
Uses PREVIOUS‑DAY CLOSING PRICES for everything (price on date D is close of D-1).
...
def get_spy_value(initial=100.0):
    """Get SPY value using previous‑day closing price."""
    try:
...
def get_all_shifted_prices(date, shifted_calendar_df):
    """Get all prices from shifted calendar DataFrame for a given date."""
    prices = {}
...
# === REBALANCE FUNCTIONS (already defined) ===

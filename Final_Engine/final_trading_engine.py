...
All pound values rounded to nearest penny (2 decimal places).
"""
...
def log_portfolio(date, value, weights, cash_pounds, realised, total_wealth):
    ...
    for t in TICKERS:
        entry[f'{t}_value'] = round(value * weights.get(t, 0.0), 2)
    log_entry(PORTFOLIO_LOG_FILE, entry, ['portfolio_value', 'cash_pounds', 'realised_profit'])
...
def log_rebalance_event(date, target, cash_pounds, value, cost, reason):
    ...
    for t in TICKERS:
        entry[f'{t}_target_value'] = round(value * target.get(t, 0.0), 2)
    log_entry(REBALANCE_LOG_FILE, entry, ['cash_pounds', 'portfolio_value', 'rebalance_reason'])
...
def log_take_profit_event(date, profit, new_active, total_realised):
    log_entry(TAKE_PROFIT_LOG_FILE, {
        'date': date.strftime("%Y-%m-%d %H:%M:%S"),
        'profit_withdrawn': round(profit, 2),
        'new_active_value': round(new_active, 2),
        'total_realised_profit': round(total_realised, 2)
    })
...
def get_latest_prices():
    """Get latest prices with holiday handling (forward-fill for closed markets)."""
    prices = {}
    for t in TICKERS:
        try:
...
def update_portfolio_value(prices, weights, cash_percentage, value, daily_rate, last_date):
    """Update portfolio value using latest prices with holiday handling."""
    ...
    for t in TICKERS:
        ...
        if w > 0.001:
            ...
            if prev_price > 0:
                equity_return += w * (curr / prev_price - 1)
    
    total_return = equity_return * (1 - cash_percentage) + daily_rate * cash_percentage
    return value * (1 + total_return), today
...
def generate_orders(value, current, target):
    orders, total = {}, 0.0
    for t in TICKERS:
        target_value = round(value * target.get(t, 0.0), 2)
        current_value = round(value * current.get(t, 0.0), 2)
        diff = target_value - current_value
        if abs(diff) > 0.01:
            orders[t] = {'action': 'BUY' if diff > 0 else 'SELL', 'amount': round(abs(diff), 2)}
            total += abs(diff)
    return orders, round(total * ROUND_TRIP_COST_PCT, 2)
...
def main():
    ...
    if is_first:
        value, cash_percentage, weights = INITIAL_CAPITAL, 0.0, {t: 0.0 for t in TICKERS}
        cash_pounds = 0.0
        ...
        cash_percentage = get_last_cash_percentage()
        cash_pounds = get_last_cash()
        print(f"   Last: £{value:.2f}, Realised: £{realised:.2f}")
        old = value
        value, new_date = update_portfolio_value(prices, weights, cash_percentage, value, daily_rate, last_update)
    ...
    if avg_vol <= CASH_MIN_VOLATILITY:
        ...
        cash_percentage = cash_cap
    else:
        cash_percentage = ((avg_vol - CASH_MIN_VOLATILITY) / (CASH_MAX_VOLATILITY - CASH_MIN_VOLATILITY)) * cash_cap
    
    cash_pounds = value * cash_percentage
    print(f"   f*: {f_star:.3f}, Cash: {cash_percentage*100:.1f}% (£{cash_pounds:.2f}), Cap: {cash_cap*100:.0f}%")
    
    adjusted_target = {t: w * (1 - cash_percentage) for t, w in target_weights.items()}
    ...
    if rebalance_needed:
        ...
        orders, cost = generate_orders(value, weights, adjusted_target)
        print(f"   Value: £{value:.2f}, Cash: {cash_percentage*100:.1f}% (£{cash_pounds:.2f}), Reason: {reason}")
        if orders:
            for t, o in orders.items():
                print(f"   {o['action']} {t}: £{o['amount']:.2f}")
        print(f"   Cost: £{cost:.4f} ({ROUND_TRIP_COST_PCT*100:.1f}%)")
        
        save_state(LAST_REBALANCE_FILE, now_date.strftime("%Y-%m-%d"))
        log_rebalance_event(now, adjusted_target, cash_pounds, value, cost, reason)

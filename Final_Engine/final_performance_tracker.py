...
def run_universe_tracker(tickers, params, name, raw_prices_df, spy_shifted=None):
    ...
    print(f"   Initial raw total: £{total:.2f} (Cash: £{cash:.2f})")

    # --- Normalise to 100 while keeping cash unchanged ---
    target_total = 100.0
    if abs(total - target_total) > 0.01:
        asset_total = sum(asset_values.values())
        if asset_total > 0:
            scale = (target_total - cash) / asset_total
            for t in asset_values:
                asset_values[t] *= scale
        else:
            # If no assets, distribute the asset portion evenly
            for t in tickers:
                asset_values[t] = (target_total - cash) / len(tickers)
        total = sum(asset_values.values()) + cash

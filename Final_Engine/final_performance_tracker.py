...
OUTPUTS: Daily asset values for all portfolios (CSV files).
"""
...
# ============================================================================
# GET CORRECT INJECTION STATE FOR EACH UNIVERSE
# ============================================================================

def get_injection_state_for_universe(tickers, params, prices_df):
    """
    Get the correct injection state for a universe.
    Uses the injection date from the log (July 28).
    """
    ...
    # Get returns up to the injection date
    ...
    if len(returns) < params['lookback_days'] * 0.5:
        print(f"   ⚠️ Insufficient data ({len(returns)} days). Using equal weights.")
        n = len(tickers)
    ...
    # Calculate cash allocation using the universe's Kelly lookback
...
def run_universe_tracker(tickers, params, name, prices_df):
    ...
    # Get the correct injection state for this universe
    injection_date, asset_values, cash = get_injection_state_for_universe(tickers, params, prices_df)
...
def main():
    ...
    for name in ["original_18", "optimal_standard", "optimal_ethical"]:
        log_file = os.path.join(FINAL_LOGS_DIR, f"{name}_daily.csv")
        if os.path.exists(log_file):
            os.remove(log_file)
            print(f"   🗑️ Removed old {name}_daily.csv")
    ...
    # SPY
    print("\n📊 Downloading SPY...")
    first_injection_date = list(injection_dates.values())[0]
    backtest_start = first_injection_date - timedelta(days=30)
    spy = download_spy(backtest_start.strftime("%Y-%m-%d"), datetime.now().strftime("%Y-%m-%d"))
    all_data["SPY"] = normalise(spy, first_injection_date)

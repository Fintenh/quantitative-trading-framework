...
# ============================================================================
# DOWNLOAD SPY WITH FALLBACK
# ============================================================================

def download_spy_with_fallback(start_date, end_date):
    """Download SPY data with fallback to S&P 500 index."""
    
    # Try SPY first
    spy_data = yf.download("SPY", start=start_date, end=end_date, progress=False)
    
    if len(spy_data) == 0:
        print("  ⚠️ No SPY data found, trying ^GSPC...")
        spy_data = yf.download("^GSPC", start=start_date, end=end_date, progress=False)
    
    if len(spy_data) == 0:
        print("  ⚠️ No S&P 500 data found, using synthetic SPY...")
        # Create synthetic SPY data (flat line at 100)
        dates = pd.date_range(start=start_date, end=end_date, freq='B')
        spy_data = pd.DataFrame({'Close': [100.0] * len(dates)}, index=dates)
...
def main():
    ...
    # Run backtests using FINAL portfolios from ethical_config
    ...
    std = run_backtest_for_universe(
        OPTIMAL_STANDARD_PORTFOLIO, OPTIMAL_STANDARD_PARAMS, backtest_start_str, end_date, OPTIMAL_STANDARD_PARAMS['kelly_lookback']
    )
    
    print("📊 Running Optimal_Ethical...")
    eth = run_backtest_for_universe(
        OPTIMAL_ETHICAL_PORTFOLIO, OPTIMAL_ETHICAL_PARAMS, backtest_start_str, end_date, OPTIMAL_ETHICAL_PARAMS['kelly_lookback']
    )
    
    # Get SPY from injection date with fallback
    print("📊 Downloading SPY benchmark data...")
    spy_data = download_spy_with_fallback(injection_date_str, end_date)
    spy = (spy_data['Close'] / spy_data['Close'].iloc[0]) * INITIAL_CAPITAL
    ...
    # Also normalise SPY to start at £100 on injection date
    spy = normalise_from_injection(spy, injection_date)
    ...
    print(f"\n📊 Period: {injection_date} to {spy.index[-1].date() if len(spy) > 0 else 'today'}")

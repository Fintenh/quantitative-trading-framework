...
rather than individual asset statistics, and tests different fractions:
  - 100% (Full Kelly)
  - 75% (Aggressive)
  - 50% (Moderate)
  - 25% (Conservative)
...
# ---------------------------------------------------------------------------
# Backtest engine with fractional Kelly
# ---------------------------------------------------------------------------

class BacktestEngineWithFractionalKelly(BacktestEngine):
    """
    Extended BacktestEngine that uses the Fractional Kelly Criterion,
    calculated from equal-weight portfolio returns rather than individual
    asset statistics.
    """
    ...
    def _calc_cash_allocation(self, returns):
        ...
        Fractional Kelly is then applied to the cash cap.
        """
        try:
        ...
        # 11. Cash allocation based on GARCH volatility (bounded by the Kelly cap)
...
if len(all_results) > 0:
    ...
    # Convert to a numpy array for arithmetic operations on the tick positions
    ...
    # Plot 6: mean f* vs Kelly fraction
    ax = axes[1, 2]

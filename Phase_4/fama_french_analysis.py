...
Analyses what drives the portfolio's returns using correctly annualised
alpha, proper significance testing, and robust handling of the factor data.
"""
...
print("=" * 70)
print("PHASE 4: FAMA-FRENCH FACTOR ANALYSIS")
print("=" * 70)
...
def build_approximate_factors():
    ...
    This is a fallback if the direct download fails.
    """
    print("\nBuilding approximate factors from Yahoo Finance...")
    ...
    try:
        ...
        # Market factor (Mkt-RF): SPY return minus the risk-free rate
        ...
        # RMW: Quality (QUAL - SPY) as a proxy for profitability
        factors['RMW'] = returns['QUAL'] - returns['SPY']

        # CMA: Conservative - Aggressive (approximated with SHY - SPY)
        ...
        print("   Note: using approximated factors")
...
# ---------------------------------------------------------------------------
# Function to get portfolio returns from the backtest
# ---------------------------------------------------------------------------
...
def get_portfolio_returns():
    ...
    try:
        ...
        # Generate synthetic returns if the backtest fails
        np.random.seed(42)
...
def perform_factor_regression(portfolio_returns, factors):
    """
    Perform a Fama-French 5-Factor regression with proper statistics.
    ...
    if len(aligned) < 100:
        print("   Insufficient observations for a reliable regression")
        return None, None
    ...
    alpha_annual = (1 + alpha_daily) ** 252 - 1  # compound annualisation, not alpha_daily * 252
...
def print_factor_results(results):
    ...
    if results['alpha_pvalue'] < 0.05:
        ...
        print("  Consistent with genuine skill, not just luck")
    else:
        print("  Alpha is NOT statistically significant (p >= 0.05)")

...
analysis.py
------------
Loads the saved Apple data and performs advanced statistical analysis,
including Phase 1.1 (probability & statistics) and Phase 1.2 (time series analysis).
"""
...
if jb_p < 0.05:
    ...
    print("→ Fail to reject H₀: Returns could be normally distributed")
...
if adf_result[1] < 0.05:
    print("→ Reject H₀: Returns are STATIONARY ✓")
    print("  (Validates using ARIMA and GARCH models.)")
else:
    print("→ Fail to reject H₀: Returns are NON-STATIONARY")
...
# Extract parameters (convert back from scaled)
...
print(f"  γ₀ (Omega): {gamma_0:.6f}  (baseline variance)")
print(f"  α₁ (Alpha): {alpha_1:.6f}  (reaction to shocks)")
print(f"  β₁ (Beta):  {beta_1:.6f}  (persistence)")
print(f"  α₁ + β₁:    {persistence:.6f}  (total persistence)")

if persistence < 1:
    print("  → Stationarity constraint satisfied ✓")
else:
    print("  → WARNING: α₁ + β₁ >= 1 (model may be non-stationary)")

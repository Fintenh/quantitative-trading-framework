...
Includes deterministic regime probability function.
GARCH output is always silenced.
Used by both hmm_run.py and hmm_optimise.py.
"""
...
# ---------------------------------------------------------------------------
# Deterministic Regime Probabilities (softmax on SPY returns)
# ---------------------------------------------------------------------------

def get_regime_probabilities(date, spy_data):
    """
    Compute smooth regime probabilities using softmax on SPY returns and volatility.
    Identical to the function previously defined in hmm_run.py and hmm_optimise.py.
    """
...
class HMMEngineWithKelly(BacktestEngineWithKelly):
    """
    HMM regime‑weighted engine with Kelly cash allocation.
    Blends rebalancing parameters using regime probabilities from SPY.
    Accepts spy_data and regime_params (dict) in __init__.
    GARCH output is ALWAYS silenced.
    Set verbose=True to print rebalance details.
    Emergency rebalance on Kelly sign change is included.
    """
    def __init__(self, spy_data, regime_params, verbose=False, **kwargs):
    ...
    def _get_blended_params(self, date):
        """Compute blended parameters from the stored regime_params."""
        probs = get_regime_probabilities(date, self.spy_data)

"""
trading_engine.py - Main execution script for Phase 2 trading system.
...
def log_rebalance(date, target_weights, cash, value, cost=0.0, reason=""):
    ...
    # Convert numpy floats to Python floats for clean CSV
...
def should_rebalance(current_date, last_date, current_weights, target_weights):
    ...
    # EMERGENCY: Extreme drift overrides min days
    max_drift, drifting_asset = calculate_drift(current_weights, target_weights)
    if max_drift > 0.10:  # 10% emergency threshold
        return True, f"Emergency: {drifting_asset} drifted {max_drift*100:.2f}%", max_drift, drifting_asset
...
def main():
    ...
    # ------------------------------------------------------------------------
    # Calculate Kelly cash cap using last KELLY_LOOKBACK days of returns

...
def select_best_garch_model(returns, max_p=2, max_q=2):
    """
    Select the best GARCH model by AIC.

    Tests GARCH, GJR-GARCH, and EGARCH across a range of orders.

    Returns:
        tuple: (model_name, fitted_model, aic)
    """
        ...
        try:
            ...
            if fitted_model is not None:
                ...
                if 'alpha[1]' in fitted_model.params:
                    ...
                    print(f"    alpha1 = {alpha1:.4f}, beta1 = {beta1:.4f}, alpha1+beta1 = {persistence:.4f}")
            else:
                ...
                print(f"  Fallback: GARCH(1,1) (AIC = {fitted.aic:.2f})")
            ...
            try:
                ...
                model_names[ticker] = 'GARCH(1,1)'
                aic_values[ticker] = fitted.aic
                print(f"  Error fallback: GARCH(1,1)")
            except:
                models[ticker] = None
...
def get_latest_volatility(models, returns):
    """
    Get the latest annualised volatility for each asset.
